...
namespace DQ
{
    ...
    ID3D12Device8* _getDevice(IDXGIAdapter* pAdapter = nullptr)
    {
#ifdef _DEBUG
    ...
    public:
        ...
        void ExecuteList(ICommandList* list)
        {
            ID3D12CommandList* lists[1] = { list->GetList() };
        ...
        void Wait(COMMAND_LIST_TYPE type)
        {
            if (type == COMMAND_LIST_TYPE::GRAPHICS)
            {
                _wait(mGFenceValue, pGraphicsQueue.Get(), pGFence.Get(), mGEvent.Get());
    ...
    {
        return std::shared_ptr<IDevice>(new Device(hwnd, width, height));
    }
}